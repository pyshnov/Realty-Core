...
namespace Pyshnov\vk;


use Pyshnov\vk\Exception\VkException;
...
class Vk
{
     ...
     * @return array|string - выводит массив данных или ошибку (но тоже в массиве)
     * @throws VkException
     */
    ...
    */
    ...
    public function upload_photo($gid = 0, array $files = [], $return_ids = false, array $additional_data = [], $usleep = 0)
    {
        ...
        if (!function_exists('curl_init')) return false;
        $data_json = $this->api('photos.getWallUploadServer', array('group_id' => $gid));
        ...
        foreach ($temp as $chunk_index => $temp_chunk) {
            ...
            foreach ($temp_chunk as $key => $data) {
                $path = realpath($data);
                if ($path) {
                    $files['file' . ($key + 1)] = new \CURLFile(realpath($data));
                }
            }
